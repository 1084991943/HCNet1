...
from Attention import CoordAtt, CBAM, SE_Block
...
class CAFC(nn.Module):
    def __init__(self):
        super(CAFC, self).__init__()
        self.factor = round(1.0/(28*28), 3)

    def forward(self, fm1, fm2):
        b, c, w1, h1 = fm1.shape
        self.attention1 = CoordAtt(c,c)
        fm1 = self.attention1(fm1)
        _, c1, w2, h2 = fm2.shape
        self.attention2 = CoordAtt(c1, c1)
        fm2 = self.attention2(fm2)
...
class HCNet(nn.Module):
    def __init__(self, class_num, arch='convnext_base'):
        super(HCNet, self).__init__()
        feature_size = 512
        if arch == 'convnext_base':
            self.features = ConvNext()
            chans = [256, 512, 1024]
        self.pool = TopkPool()
